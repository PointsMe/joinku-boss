  ...
  <el-card :body-style="{ padding: '0px' }">
      <div style="padding: 20px 20px; min-height: 120px">
        <el-form size="mini" ref="bindForm" :model="bindForm" :rules="bindFormRules" label-width="140px;">
          <el-form-item :label="$t('common.shopId')+':'" style="text-align: left;" prop="mid">
            <el-input style="width: 320px;" :placeholder="$t('common.inpBindShopId')" v-model.trim="bindForm.mid"
                      class="input-with-select">
              <el-button slot="append" icon="el-icon-search" @click="searchPointsMeMch">{{$t('common.searchShop')}}</el-button>
            </el-input>
...
<script>
  import {
    merchantRelationList,
    merchantRelationPrebind,
    lenderPointsmeBind,
    lenderPointsmeUntied
  } from '@/api/api';
  export default {
    ...
    data() {
      return {
        bindFormRules: {
          mid: [{required: true, message: this.$t('common.inpJoyShopId'), trigger: 'blur'}]
        },
        bindForm: {
          mid: '',
        },
    ...
    methods: {
      loadPointsMeMidList() {
        ...
        params.append('source', 103);
        this.pointsMeMidList = [];
        merchantRelationList(params).then(resp => {
      ...
      searchPointsMeMch() {
        ...
        this.$refs.bindForm.validate(valid => {
          if (valid) {
            ...
            merchantRelationPrebind(params).then(resp => {
              if (resp.code === 20000) {
                _this.alert.visible = true
                _this.alert.type = 'success'
                _this.alert.title = this.$t('common.shopName') + resp.data.name
                _this.enableBind = true
              } else {
                _this.alert.visible = true
                _this.alert.type = 'warning'
                _this.alert.title = 'Joyingo' + this.$t('common.shopInexis')
      ...
      bindPointsMeMch() {
        ...
        lenderPointsmeBind(params).then(resp => {
          if (resp.code === 20000) {
      ...
      unbindPointsMeMch(id) {
        ...
        lenderPointsmeUntied(params).then(resp => {
          if (resp.code === 20000) {
