  ...
  <script>
    ...
    export default {
      ...
      data() {
        return {
          bindFormRules: {
            mid: [{required: true, message: this.$t('common.inputMerMidHint'), trigger: 'blur'}]
          },
          bindForm: {
            mid: '',
          },
      ...
      methods: {
        loadPointsMeMidList() {
          ...
          params.append('source', 111);
          this.pointsMeMidList = [];
          lenderPointsmeList(params).then(resp => {
        ...
        searchPointsMeMch() {
          ...
          this.$refs.bindForm.validate(valid => {
            if (valid) {
              let params = new URLSearchParams();
              params.append('mid', this.bindForm.mid);
              params.append('source', 111);
              lenderAmySearch(params).then(resp => {
                if (resp.code === 20000) {
                  ...
                  _this.alert.title = 'AmySushi' + this.$t('common.merInexis')
        ...
        bindPointsMeMch() {
          let _this = this;
          let params = new URLSearchParams();
          params.append('merchantId', _this.bindForm.mid);
          params.append('lenderId', _this.id);
          params.append('source', 111);
