              ...
              <div v-if="inv.pensions && inv.pensions.length>0">
                ...
                >
                  <div>
                    ...
                    <div>
                      Aliquota cassa:
                      {{item.rate}}% su {{item.percent}}% {{item.withholding | withholdingValue}} {{item.vatId | vatValue(vatList)}}
                    </div>
                  </div>
                </div>
              </div>
...
const INVOICE_PAYMENT_CHANNEL_LIST = require("@/assets/data/invoice-payment-channel.json");
  import {
    invoiceReceivedDetail,getVatList
  } from '@/api/api';
...
export default {
  ...
  methods: {
    loadSentDetail() {
      ...
      invoiceReceivedDetail(params).then(resp => {
        if (resp.code === 20000) {
          _this.inv = resp.data;
