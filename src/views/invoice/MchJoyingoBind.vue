  ...
  <el-card :body-style="{ padding: '0px' }">
      <div style="padding: 20px 20px; min-height: 120px">
        <el-form size="mini" ref="bindForm" :model="bindForm" :rules="bindFormRules" label-width="140px;">
          <el-form-item :label="$t('common.merchantId2')" style="text-align: left;" prop="mid">
            <el-input style="width: 320px;" :placeholder="$t('common.inpBindMerHint')" v-model.trim="bindForm.mid"
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        bindFormRules: {
          mid: [{required: true, message: this.$t('common.inpJoyMerId'), trigger: 'blur'}]
        },
        bindForm: {
          mid: '',
        },
          ...
          if (valid) {
            ...
            merchantRelationPrebind(params).then(resp => {
              if (resp.code === 20000) {
                ...
                _this.alert.title = 'Joyingo' + this.$t('common.merInexis')
